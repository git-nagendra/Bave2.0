...
document.addEventListener("DOMContentLoaded", function () {
  ...
  toggleBtn.addEventListener("click", function () {
      if (moreText.classList.contains("hidden")) {
          ...
          toggleBtn.textContent = "...Red More";
      }
  });  
});
...
// counter js 
document.addEventListener("DOMContentLoaded", function () {
  function setupCounter(containerSelector, inputSelector, minusSelector, plusSelector) {
    document.querySelectorAll(containerSelector).forEach((counter) => {
      let input = counter.querySelector(inputSelector);
      let minus = counter.querySelector(minusSelector);
      let plus = counter.querySelector(plusSelector);
  ...
  // Setup both counters
  setupCounter(".input-step", ".product-quantity", ".minus", ".plus");
  setupCounter(".counter", ".quantity", ".round:first-of-type", ".round:last-of-type");
});
...
document.addEventListener("DOMContentLoaded", function () {
  ...
  serviceItems.forEach((item) => {
    const servicesLink = item.querySelector(
...
function setPanelFocusability(panel, focusable) {
  panel
    ...
    .forEach((el) => (el.tabIndex = focusable ? 0 : -1));
}

document
  .querySelectorAll(".tabs-content .tab-content")
  .forEach((panel) => setPanelFocusability(panel, false));
...
function enterPanelFocusTrap(panel, tabButton) {
  ...
  let focusableElements = [...panel.querySelectorAll(focusableSelector)].filter(
    (el) => el.offsetParent
  );
  ...
  function panelKeydownHandler(e) {
    if (["ArrowUp", "Escape"].includes(e.key)) {
      ...
      focusableElements = [...panel.querySelectorAll(focusableSelector)].filter(
        (el) => el.offsetParent
      );
  ...
  tab.addEventListener("keydown", (e) => {
    if (e.key === "ArrowDown") {
      ...
      const panel = document.querySelector(
        `.tabs-content .tab-content[data-tab="${tab.dataset.tab}"]`
      );
...
function changeActiveTab(target) {
  ...
  document
    .querySelector(`.tab-button[data-tab="${targetTab}"]`)
